            ...
            try:
                ...
                if not integration:
                    results["errors"].append(
                        f"Integration {integration_id} not found or disabled"
                    )
                    continue
                ...
                results["deliveries_failed"] += 1
                results["errors"].append(
                    f"Error delivering to {integration_id}: {str(e)}"
                )
                logger.exception(
                    f"Error routing alert {alert_event_id} to {integration_id}"
                )
    ...
    async def deliver_teams(
        ...
        try:
            ...
            payload = {
                ...
                "themeColor": (
                    "FF0000" if alert_event.severity == "critical" else "0078D7"
                ),
                "summary": alert_event.title,
                "sections": [
                    {
