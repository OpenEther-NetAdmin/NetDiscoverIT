...
async def detect_config_drift(
    ...
    """
    from app.models.models import ChangeRecord, AlertRule, AlertEvent, Device
    ...
    for record in change_records:
        if record.post_change_hash == new_config_hash:
            if record.status == "scheduled":
                if (record.scheduled_window_start <= captured_at <= record.scheduled_window_end):
                    authorized = True
            ...
            for record in change_records:
                if record.status in ["approved", "scheduled"]:
                    if record.status == "scheduled" and record.scheduled_window_end:
                        if record.scheduled_window_end.replace(tzinfo=None) > grace_period_end:
                            grace_period_end = record.scheduled_window_end.replace(tzinfo=None)
