...
class DeviceVectorizer:
    ...
    def generate_vectors(self, device_metadata: Dict) -> Dict:
        """Generate vector embeddings for a device"""
        vectors = {}

        vectors['device_role'] = self._generate_role_vector(device_metadata)
        vectors['topology'] = self._generate_topology_vector(device_metadata)
        vectors['security'] = self._generate_security_vector(device_metadata)
        vectors['config'] = self._generate_config_vector(device_metadata)

        return vectors

    def _generate_role_vector(self, metadata: Dict) -> List[float]:
    ...
    def _generate_topology_vector(self, metadata: Dict) -> List[float]:
    ...
    def _generate_security_vector(self, metadata: Dict) -> List[float]:
    ...
    def _generate_config_vector(self, metadata: Dict) -> List[float]:
    ...
    def _rule_based_role_vector(self, metadata: Dict) -> List[float]:
        ...
        for i in range(VECTOR_DIM):
            vector[i] = ((seed * (i + 1)) % VECTOR_HASH_RANGE) / VECTOR_HASH_RANGE

        return vector

    def _rule_based_topology_vector(self, metadata: Dict) -> List[float]:
        """Generate rule-based topology vector"""
        vector = [0.0] * VECTOR_DIM
        ...
        seed_input = f"{hostname}{len(interfaces)}{len(vlans)}"
    ...
    def _rule_based_security_vector(self, metadata: Dict) -> List[float]:
        """Generate rule-based security vector"""
        vector = [0.0] * VECTOR_DIM

        acls = metadata.get('acls', [])
        zones = metadata.get('zones', [])
        nat = metadata.get('nat', False)
        
        seed_input = f"{len(acls)}{len(zones)}{nat}"
        seed = int(hashlib.md5(seed_input.encode()).hexdigest()[:8], 16)
