...
async def retry_failed_deliveries(
    ...
    query = select(AlertEvent).where(AlertEvent.notifications_sent.isnot(None))
    ...
    for event in events:
        ...
        failed_integrations = [
            n["integration_id"] for n in notifications if n.get("status") == "failed"
        ]
        ...
        if failed_integrations:
            try:
                ...
                results["deliveries_succeeded"] += delivery_result.get(
                    "deliveries_succeeded", 0
                )
                results["deliveries_failed"] += delivery_result.get(
                    "deliveries_failed", 0
                )
