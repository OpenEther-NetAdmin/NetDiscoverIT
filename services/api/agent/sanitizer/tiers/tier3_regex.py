...
class AggressiveRegexSanitizer:
    ...
    PATTERNS: List[Tuple[re.Pattern, TokenType, str]] = [
        ...
        # SNMP communities
        (
            re.compile(r"snmp-server community\s+(\S+)", re.IGNORECASE),
            TokenType.COMMUNITY,
            "SNMP community",
        ),
        (
        ...
        # BGP AS numbers
        (
            re.compile(r"router\s+bgp\s+(\d+)", re.IGNORECASE),
            TokenType.BGP_AS,
            "BGP AS",
        ),
        # VLAN IDs
    ...
    def sanitize(self, config_text: str) -> Tier3Result:
        ...
        return Tier3Result(
            sanitized_text="\n".join(sanitized_lines), redactions=redactions
        )
